const showInternalCommunicationsDetails = (details) => {
    let popupContent = new InternalCommunicationsDetails(details).init();
    $("#details-modal-container").empty().html(popupContent);
}


class InternalCommunicationsDetails{
    constructor(details){
        this.chartColors = [
    ...
    countryWiseChart(chartData){
        if(chartData.length){
            Highcharts.chart("details-country-wise", {
                ...
                series: [
                    {
                        "name": "Internal Communications, Feedback, Review  and Engagement",
