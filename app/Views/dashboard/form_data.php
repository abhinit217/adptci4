...
<div class="app-content page-body">
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-12">
			<!-- <button type="button" class="btn btn-sm btn-warning pull-right send_back" onclick="downloaddata();" >Download</button> -->
			<!-- <a href="#"  onclick="download_data();" class="btn btn-sm btn-success pull-right download_data">Download</a> -->
								...
								<div class="card">
									<div class="card-body">
										<div class="row" id="data_<?php echo $key; ?>">
											...
											<div class="col-md-12">
												<?php if(($data['user_id'] == $this->session->userdata('login_id') && ($data['status'] == 2 || $data['status'] == 1 || $data['status'] == 4)) && ($this->session->userdata('role') == 6 || $this->session->userdata('role') == 5 || $this->session->userdata('role') == 4 || $this->session->userdata('role') == 3)){ ?>
												...
												<!-- <?php if($this->session->userdata('role') == 5 && $data['status'] != 3){ ?>
													...
													<a href="<?php echo base_url(); ?>reporting/edit_data/<?php echo $data['form_id'];?>/<?php echo $data['year_id'];?>/<?php echo $data['id'];?>" target="_blank" class="pull-right " style="color: blue; margin-right: 15px;" ><i class="fa fa-pencil" style="font-size:18px;"></i> Edit data</a>
												<?php } ?> -->
																	...
																	switch ($field['type']) {
																		...
																		case 'lkp_cluster':
																			...
																			$lkpdata= "<td></td>";
																				...
																				foreach ($lkp_cluster as $lkp_key => $country) {
																					if($country['cluster_id'] == $data_array[$field_key]){
																						$lkpdata = "<td>".$country['cluster_name']."</td>";
																					}
																				}
																		...
																		case 'lkp_country':
																			...
																			if($field['multiple']) {
																				...
																				$lkp_str_arr = explode(",", (string) $data_array[$field_key]);
																				$lkpdata= $lkp_str_arr;
																				foreach ($lkp_str_arr as $lkp_str_key => $str_country) {
																					$lkpdata = "<td>".$str_country."</td>";
																				}
																			}else{
																				foreach ($lkp_country as $lkp_key => $country) {
																					if($country['country_id'] == $data_array[$field_key]){
																						$lkpdata = "<td>".$country['country_name']."</td>";
																					}
																				}
																			}
											...
											<div class="col-md-12 mt-10">
												<?php if (isset($group_array) && (count($data) > 0)) {
													foreach ($group_array as $key => $group) { ?>
														...
														<a href="<?php echo base_url(); ?>reports/edit_groupdata_info/<?php echo $data['form_id'];?>/<?php echo $data['data_id'];?>/<?php echo $group['group_field_id'];?>/<?php echo $data['status'];?>" target="_blank" class="pull-right " style="color: blue; margin-right: 15px;" ><i class="fa fa-pencil" style="font-size:18px;"></i> Edit group data</a>
														<div class="table-responsive">
									        				<table class="table table-bordered" id="gdatatable">
																<thead>
																	<tr style="background-color:#050c43;">
																		<th>Sl.no</th>
...
<script type="text/javascript">
    ...
    //                     return false;
    //                 }else{
	// 					var records_list =response.json_data;
	// 					var excelgdata =response.g_json_data;
	// 					var fields ={};
	// 					response.form_fields.forEach(d=>fields[`field_${d.field_id}`]=d.label);
	// 					// response.form_fields.forEach(dg=>fields[`field_${dg.field_id}`]=dg.label);
	...
	// 					const ws2 = XLSX.utils.json_to_sheet(excelgdata);
	// 					XLSX.utils.book_append_sheet(wb, ws2, 'Sheet2');
	// 					XLSX.writeFile(wb, 'excel_file_name.xlsx');
	// 				}
