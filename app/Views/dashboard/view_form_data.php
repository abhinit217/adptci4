								...
								<div class="card">
									<div class="card-body">
										<div class="row" id="data_<?php echo $key; ?>">
												...
												<div class="row">
													...
													<div class="col-md-2">
														<?php if($this->uri->segment(5) == 4){?>
															<p class="pull-left">Rejected by: <?php echo $data['useaprrname']; ?></p>
														<?php }else{?>
											...
											<div class="col-md-12">
												<?php if($data['user_id'] == $this->session->userdata('login_id') && $data['status'] != 3 ){ ?>
												...
												<?php if($data['status'] == 2 && ($this->session->userdata('role') != 6 || ($data['user_id'] == $this->session->userdata('login_id')))){ ?>
													...
													<!-- <a href="<?php echo base_url(); ?>reporting/edit_data/<?php echo $data['form_id'];?>/<?php echo $data['year_id'];?>/<?php echo $data['id'];?>" target="_blank" class="pull-right " style="color: blue; margin-right: 15px;" ><i class="fa fa-pencil" style="font-size:18px;"></i> Edit</a> -->
												<?php } ?>
																...
																<?php foreach ($form_fields as $f_key => $field) {
																	...
																	switch ($field['type']) {
																		...
																		case 'lkp_country':
																			...
																			$lkpdata= "";
																			if($field['multiple']) {
																				$lkp_str_arr = explode("&#44;", $data_array[$field_key]);
																				$lkpdata .= "<td>";
																		...
																		foreach ($data['groupdata'] as $gkey => $g_data) { 
																			...
																			if($group['group_lable']['field_id'] == $g_data['groupfield_id']){ ?>
																				<tr id="data_<?php echo $key; ?><?php echo $gkey; ?>">
																					...
																					<?php }
																					if(($data['user_id'] == $this->session->userdata('login_id') && $data['status'] != 3 )){ ?>
																						<td>
...
<script type="text/javascript">
	...
	// 					});
	// 					var gfields ={};
	// 					response.form_g_fields.forEach(dg=>gfields[`field_${dg.field_id}`]=dg.label);
	// 					var excelgdata =response.g_json_data.map(dg=> {
	// 						const glist ={};
	// 						Object.keys(dg).forEach(kg=>glist[gfields[kg] || kg]=dg[kg]);
	// 						return glist;
	// 					});
