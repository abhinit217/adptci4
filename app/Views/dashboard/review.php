<style type="text/css">
	.form-control button { border: none !important; }
	.form-control span { margin: 5px; }
	.ms-drop.bottom { left: 0; }

	span.badge > span {
		width: 100%;
		min-width: 40px;
		text-align: center;
		display: grid;
		margin: 0 auto;
		margin-top: 5px;
	}

	/* loading dots */
	.loading:after {
		content: ' .';
		padding-right: 5px;
		animation: dots 1s steps(5, end) infinite;
	}
	@keyframes dots {
		0%, 20% {
			color: rgba(255,255,255,0);
			text-shadow:
			.25em 0 0 rgba(255,255,255,0),
			.5em 0 0 rgba(255,255,255,0);
		}
		40% {
			color: black;
			text-shadow:
			.25em 0 0 rgba(255,255,255,0),
			.5em 0 0 rgba(255,255,255,0);
		}
		60% {
			text-shadow:
			.25em 0 0 black,
			.5em 0 0 rgba(255,255,255,0);
		}
		80%, 100% {
			text-shadow:
			.25em 0 0 black,
			.5em 0 0 black;
		}
	}
</style>
<link href="<?php echo base_url(); ?>include/assets/plugins/wysiwyag/richtext.css" rel="stylesheet" />

<div id="synthesisReportModal" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		<!-- Modal content-->
		<div class="modal-content">
			<div class="modal-header">
				<h3 class="modal-title">SYNTHESIS REPORT</h3>
			</div>
			
			<div class="modal-body">
				<textarea name="report" rows="6" class="form-control" id="textarea_1"></textarea>
				<p class="error red-800"></p>
				<p class="maxlengtherror red-800"></p>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-danger pull-left" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-success submit_summaryreport">Submit Report</button>
			</div>
		</div>
	</div>
</div>

<div class="app-content page-body">
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-12">
				<h3>Review Dashboard</h3>
				
				<!-- Year Country Crop -->
				<div class="card">
					<div class="card-body">
						<div class="row">
							<div class="col-xs-4 col-md-4 col-lg-4 col-sm-12">
								<div class="form-group">
									<label class="form-label">Select Year</label>
									<select name="year" placeholder="Select Year" class="form-control">
										<?php foreach ($year_list as $key => $year) {
											$selected = ($key == 0) ? 'selected' : ''; ?>
											<option value="<?php echo $year['year_id']; ?>" <?php echo $selected; ?>>
												<?php echo $year['year']; ?>
											</option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="col-xs-4 col-md-4 col-lg-4 col-sm-12">
								<div class="form-group">
									<label class="form-label">Select Country</label>
									<select name="country" placeholder="Select Country" class="form-control"></select>
								</div>
							</div>
							<div class="col-xs-4 col-md-4 col-lg-4 col-sm-12">
								<div class="form-group">
									<label class="form-label">Select Crop</label>
									<select name="crop" placeholder="Select Crop" class="form-control"></select>
								</div>
							</div>
						</div>
					</div>
				</div>

				<!-- Counters -->
				<!-- <div class="row">
					<div class="col-sm-4">
						<div class="row">
							<div class="col-sm-6">
								<div class="card">
									<div class="card-status card-status-left bg-warning br-bl-7 br-tl-7">
									</div>
									<div class="card-body">
										<span class="float-left font-weight-bold">
											<h3 class="card-title text-warning">Total Assigned :
											</h3>
										</span>
										<span class="font-weight-bold float-right">
											<h3 class="card-title text-warning">2</h3>
										</span>
									</div>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="card">
									<div class="card-status card-status-left bg-primary br-bl-7 br-tl-7">
									</div>
									<div class="card-body">
										<span class="float-left font-weight-bold">
											<h3 class="card-title text-primary">Total Reported :
											</h3>
										</span>
										<span class="font-weight-bold float-right">
											<h3 class="card-title text-primary">2</h3>
										</span>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-3">
						<div class="row">
							<div class="col-sm-12">
								<div class="card">
									<div class="card-status card-status-left bg-success br-bl-7 br-tl-7"></div>
									<div class="card-body">
										<span class="float-left font-weight-bold">
											<h3 class="card-title text-success">Pending Approval :
											</h3>
										</span>
										<span class="font-weight-bold float-right">
											<h3 class="card-title text-success">1</h3>
										</span>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-5">
						<div class="row">
							<div class="col-sm-6">
								<div class="card">
									<div class="card-status card-status-left bg-red br-bl-7 br-tl-7">
									</div>
									<div class="card-body">
										<span class="float-left font-weight-bold">
											<h3 class="card-title text-red">Total Approved :
											</h3>
										</span>
										<span class="font-weight-bold float-right">
											<h3 class="card-title text-red">1</h3>
										</span>
									</div>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="card">
									<div class="card-status card-status-left bg-info br-bl-7 br-tl-7">
									</div>
									<div class="card-body">
										<span class="float-left font-weight-bold">
											<h3 class="card-title text-info">Total Send Back :
											</h3>
										</span>
										<span class="font-weight-bold float-right">
											<h3 class="card-title text-info">1</h3>
										</span>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div> -->

				<!-- Reporting/Approval/Review Tab -->
				<div class="panel panel-primary">
					<div class="tab-menu-heading p-0 bg-light">
						<div class="tabs-menu1">
							<!-- Tabs -->
							<ul class="nav panel-tabs" style="position:relative;">
								<li><a href="<?php echo base_url() . 'dashboard'; ?>" class="text-black">REPORTING</a></li>
								
								<?php if($this->session->userdata('role') == 6){ ?>
								<li><a href="<?php echo base_url() . 'dashboard/approval'; ?>" class="text-black">APPROVAL</a></li>
								<li><a href="javascript:void(0)" class="text-black active">REVIEW</a></li>
								<li><a href="<?php echo base_url() . 'dashboard/user_uploadinfo'; ?>" class="text-black">USER UPLOAD INFO</a></li>
								<li><a href="<?php echo base_url() . 'dashboard/monitoring_evaluation'; ?>" class="text-black">M & E</a></li>
								<?php } ?>

								<?php if ($this->session->userdata('role') == 4) { ?>
								<li><a href="javascript:void(0)" class="text-black active">APPROVAL</a></li>
								<?php } else if ($this->session->userdata('role') == 5) { ?>
								<li><a href="<?php echo base_url().'dashboard/approval'; ?>" class="text-black">APPROVAL</a></li>
								<li><a href="javascript:void(0)" class="text-black active">REVIEW</a></li>
								<?php } ?>

								 <?php if($this->session->userdata('role') == 6 || $this->session->userdata('role') == 5 || $this->session->userdata('role') == 4){ ?>
								<li><a href="<?php echo base_url() . 'dashboard/result_tracker'; ?>" class="text-black">RESULTS TRACKER</a></li>
								<?php } ?>
								
								<li style="position:absolute;right:0;top:3px;">
									<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#synthesisReportModal">Write Synthesis Report</button>
								</li>
							</ul>
							
							<!-- <ul class="nav panel-tabs" style="position:relative;">
								<li><a href="<?php echo base_url().'dashboard'; ?>" class="text-black">REPORTING</a></li>
								<?php if($this->session->userdata('role') == 4) { ?>
								<li><a href="<?php echo base_url().'dashboard/approval'; ?>" class="text-black">APPROVAL</a></li>
								<?php } else if($this->session->userdata('role') == 5) { ?>
								<li><a href="<?php echo base_url().'dashboard/approval'; ?>" class="text-black">APPROVAL</a></li>
								<li><a href="javascript:void(0)" class="text-black active">REVIEW</a></li>
								<li style="position:absolute;right:0;top:3px;">
									<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#synthesisReportModal">Write Synthesis Report</button>
								</li>
								<?php } else if($this->session->userdata('role') == 6) { ?>
								<li><a href="<?php echo base_url().'dashboard/approval'; ?>" class="text-black">APPROVAL</a></li>
								<li><a href="javascript:void(0)" class="text-black active">REVIEW</a></li>
								<li style="position:absolute;right:0;top:3px;">
									<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#synthesisReportModal">Write Synthesis Report</button>
								</li>
								<?php } ?>
							</ul> -->
						</div>
					</div>
				</div>

				<!-- <div class="panel panel-primary summary_div">
					<label>SYNTHESIS REPORT</label>
                    <textarea name="report" rows="6" class="form-control" id="textarea_1"></textarea>
                    <p class="error red-800"></p>
                    <p class="maxlengtherror red-800"></p>

                    <button type="button" class="btn btn-sm btn-success pull-right submit_summaryreport">Submit</button>
				</div> -->

				<!-- PO Tab -->
				<div class="panel panel-primary mt-5">
					<div class="tab-menu-heading p-0 bg-light">
						<div class="tabs-menu1">
							<!-- Tabs -->
							<ul class="nav panel-tabs po"></ul>
						</div>
					</div>

					<div class="tabs-menu-body p-0">
						<!-- Tabs Content -->
						<div class="tab-content po"></div>
					</div>
				</div>
			</div><!-- end app-content-->
		</div>
	</div>
</div>
</div>

<script src="<?php echo base_url(); ?>include/assets/plugins/wysiwyag/jquery.richtext.js"></script>

<!-- Page Script -->
<script type="text/javascript">
	// This will encode your HTML input
    function removeTags(str) {
        if ((str===null) || (str===''))
            return false;
        else
            str = str.toString();
              
        // Regular expression to identify HTML tags in
        // the input string. Replacing the identified
        // HTML tag with a null string.
        str = str.replace( /(<([^>]+)>)/ig, '');
        return str.replace(/\&nbsp;/g, '');
    }
	$('#synthesisReportModal').on('click', '.submit_summaryreport', function(){
		$(this).closest('.modal-content').find('.error').empty();
		var year_id = $('[name="year"]').val();
		var country_id = $('[name="country"]').val();
		var crop_id = $('[name="crop"]').val();

		var value = removeTags($('#textarea_1').val());
		if ($.trim(value).length === 0) {
            $(this).closest('.modal-content').find('.error').html('This field is required');
            return false;
        }

		$.ajax({
			url: '<?php echo base_url(); ?>reporting/submit_summaryreport',
			type: 'POST',
			dataType:'JSON',
			data: { 
				year_id: year_id, 
				country_id: country_id, 
				crop_id: crop_id, 
				comment: $('#textarea_1').val(), 
			},
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}else{
					$.toast({
						stack: false,
						icon: 'success',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Success',
						text: response.msg
					});
				}
			}
		});

		/*if(country_id.length == 1 && crop_id.length == 1){
			$.ajax({
				url: '<?php echo base_url(); ?>reporting/submit_summaryreport',
				type: 'POST',
				dataType:'JSON',
				data: { 
					year_id: year_id, 
					country_id: country_id, 
					crop_id: crop_id, 
					comment: $('#textarea_1').val(), 
				},
				error: function() {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Network Error!',
						text: 'Please check your internet connection.'
					});
				},
				success: function(response) {
					if(response.status == 0) {
						$.toast({
							stack: false,
							icon: 'error',
							position: 'bottom-right',
							showHideTransition: 'slide',
							heading: 'Error!',
							text: response.msg
						});
						return false;
					}else{
						$.toast({
							stack: false,
							icon: 'success',
							position: 'bottom-right',
							showHideTransition: 'slide',
							heading: 'Success',
							text: response.msg
						});
					}
				}
			});
		}else{
			if(country_id.length > 1 && crop_id.length > 1){
				var msg = 'Please select only one country and crop for reporting data.';
			}else if(country_id.length > 1){
				var msg = 'Please select only one country for reporting data.';
			}else if(crop_id.length > 1){
				var msg = 'Please select only one crop for reporting data.';
			}else if(country_id.length == 0 && crop_id.length == 0){
				var msg = 'Please select one country and crop for reporting data.';
			}

			$.toast({
				stack: false,
				icon: 'warning',
				position: 'bottom-right',
				showHideTransition: 'slide',
				heading: 'Warning!',
				text: msg
			});
		}*/
	});

	$(function() {
		$('#textarea_1').richText();

		$('[name="year"]').on('change', function (event) {
			getCountry();
		}).multipleSelect({
			filter: true
		});

		$('[name="country"]').on('change', function (event) {
			getCrops();
		}).multipleSelect({
			filter: true
		});

		$('[name="crop"]').on('change', function (event) {
			get_summarydata();
			getPO();
		}).multipleSelect({
			filter: true
		});
		
		$('[name="year"]').trigger('change');
	});

	$('body').on('shown.bs.tab', 'a[data-toggle="tab"]', function (e) {
		var id = $(e.target).data('id');
		getPoDetails(id);
	});

	// Handle collapse icon change
	$('body').on('click', '[data-toggle="collapse"]', function(event) {
		var elem = $(this);
		if(elem.hasClass('fa-chevron-right')) {
			elem.removeClass('fa-chevron-right');
			elem.addClass('fa-chevron-down');
		} else {
			elem.removeClass('fa-chevron-down');
			elem.addClass('fa-chevron-right');
		}
	});

	function getCountry() {
		$('[name="country"]').html('');
		
		// AJAX to get country and crop
		$.ajax({
			url: '<?php echo base_url(); ?>helper/get_country',
			type: 'POST',
			dataType:'JSON',
			data: { purpose: 'review', year: $('[name="year"]').val() },
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}
				
				var countries = '';
				response.countries.forEach(function(country, index) {
					var selected = '';
					if(index == 0) selected = 'selected';
					countries += '<option value="'+country.country_id+'" '+selected+'>'+country.country_name+'</option>'
				});
				$('[name="country"]').html(countries);

				// Refresh options
				$('[name="country"]').trigger('change');
				$('[name="country"]').multipleSelect('refresh');
			}
		});
	}
	function getCrops() {
		$('[name="crop"]').html('');

		// AJAX to get crop
		$.ajax({
			url: '<?php echo base_url(); ?>helper/get_crop',
			type: 'POST',
			dataType:'JSON',
			data: {
				purpose: 'review',
				year: $('[name="year"]').val(),
				country: $('[name="country"]').val()
			},
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}
				
				var crops = '';
				response.crops.forEach(function(crop, index) {
					var selected = '';
					if(index == 0) selected = 'selected';
					crops += '<option value="'+crop.crop_id+'" '+selected+'>'+crop.crop_name+'</option>'
				});
				$('[name="crop"]').html(crops);

				// Refresh options
				$('[name="crop"]').trigger('change');
				$('[name="crop"]').multipleSelect('refresh');
			}
		});
	}

	function get_summarydata() {
		$.ajax({
			url: '<?php echo base_url(); ?>reporting/get_summarydata',
			type: 'POST',
			dataType:'JSON',
			data: {
				year: $('[name="year"]').val(),
				crop: $('[name="crop"]').val(),
				country: $('[name="country"]').val()
			},
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}else{
					if(response.comment != ''){
						var HTML = `<label>Summary report</label>
	                    <textarea name="report" rows="6" class="form-control" id="textarea_1">`+response.comment+`</textarea>
	                    <p class="error red-800"></p>
	                    <p class="maxlengtherror red-800"></p>

	                    <button type="button" class="btn btn-sm btn-success pull-right submit_summaryreport">Submit</button>`;
	                    console.log(HTML);
						$('.summary_div').html(HTML);
						$('.summary_div').find('#textarea_1').richText();
					}
				}
			}
		});
	}

	function getPO() {
		$('.panel-tabs.po').html('<li><a href="#po" class="text-black active" data-toggle="tab">PO</a></li>');
		$('.tab-content.po').html('<div class="tab-pane active" id="po">\
			<div class="row">\
				<div class="col-md-12 col-lg-12">\
					<div class="card card-block">\
						<div class="card-header d-sm-flex d-block">\
							<h3 class="card-title mr-5">Please Wait... Getting Data.</h3>\
						</div>\
					</div>\
				</div>\
			</div>\
		</div>');

		// AJAX to get po
		$.ajax({
			url: '<?php echo base_url(); ?>helper/get_po',
			type: 'POST',
			dataType:'JSON',
			data: { 
				purpose: 'review',
				year: $('[name="year"]').val(),
				crop: $('[name="crop"]').val(),
				country: $('[name="country"]').val()
			},
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}
				
				var poLis = '', poDivs = '';
				response.pos.forEach(function(po, index) {
					var active = (index == 0) ? 'active' : '';
					poLis += '<li><a href="#po'+po.po_id+'" class="text-black '+active+'" data-toggle="tab" data-id="'+po.po_id+'">'+po.po_name+'</a></li>'
					poDivs += '<div class="tab-pane '+active+'" id="po'+po.po_id+'">\
						<div class="row">\
							<div class="col-md-12 col-lg-12">\
								<div class="card card-block">\
									<div class="card-header d-sm-flex d-block">\
										<h3 class="card-title mr-5">Please Wait... Getting Data.</h3>\
									</div>\
								</div>\
							</div>\
						</div>\
					</div>'
				});
				if(response.pos.length === 0) {
					poLis = '<li><a href="#po" class="text-black active" data-toggle="tab">PO</a></li>';
					$('.tab-content.po').html('<div class="tab-pane active" id="po">\
						<div class="row">\
							<div class="col-md-12 col-lg-12">\
								<div class="card card-block">\
									<div class="card-header d-sm-flex d-block">\
										<h3 class="card-title mr-5">No PO Assigned to User.</h3>\
									</div>\
								</div>\
							</div>\
						</div>\
					</div>');
				}
				$('.panel-tabs.po').html(poLis);

				if(response.pos.length > 0) {
					$('.tab-content.po').html(poDivs);
					getPoDetails(response.pos[0].po_id);
				}
			}
		});
	}

	function getPoDetails(id) {
		$('#po'+id).html('<div class="row">\
			<div class="col-md-12 col-lg-12">\
				<div class="card card-block">\
					<div class="card-header d-sm-flex d-block">\
						<h3 class="card-title mr-5">Please Wait... Getting Data.</h3>\
					</div>\
				</div>\
			</div>\
		</div>');
		
		// AJAX to get po details
		$.ajax({
			url: '<?php echo base_url(); ?>helper/get_po_details',
			type: 'POST',
			dataType:'JSON',
			data: {
				po: id,
				purpose: 'review',
				year: $('[name="year"]').val(),
				crop: $('[name="crop"]').val(),
				country: $('[name="country"]').val()
			},
			error: function() {
				$.toast({
					stack: false,
					icon: 'error',
					position: 'bottom-right',
					showHideTransition: 'slide',
					heading: 'Network Error!',
					text: 'Please check your internet connection.'
				});
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}

				var $po = response.po_list[0];
				var user_role = <?php echo $this->session->userdata('role'); ?>;
				

				// var HTML = `<div class="row">
				// 	<div class="col-md-12 col-lg-12">
				// 		<div class="card card-block">
				// 			<div class="card-header d-sm-flex d-block">
				// 				<h3 class="card-title mr-5"> Summary ${$po['po_name']}</h3>
				// 			</div>
				// 			<div class="card-body mb-5">
				// 				<div class="table-responsive">
				// 					<table class="table table-bordered table_th">
				// 						<thead>
				// 							<tr>
				// 								<th>Output</th>
				// 							</tr>
				// 						</thead>`;
				// 						$po['output_list'].forEach(function($output, $key) {
				// 							HTML += `<tbody>
				// 								<tr>
				// 									<td>
				// 										${$output['title']}<br/><br/>
				// 										<strong style="text-decoration:underline;">Report :</strong><br/><br/>`;

				// 										if($output['report'].length > 0) {
				// 											for(var report of $output['report']){
				// 												HTML += report['report']+" by "+report['username'];
				// 												var pending = 0, responded = 0;
				// 												if(report['query_status']) {
				// 													if(report['query_status'] == 1) { pending = 1;responded = 0; }
				// 													if(report['query_status'] == 2) { pending = 0;responded = 1; }
				// 												}
				// 												HTML += `<a class="text-primary reporting ml-5" data-formid="${$output['id']}" href="javascript:void(0);">
				// 													<span class="text-success">
				// 														View Details 
				// 													</span>
				// 												</a> |
				// 												<a href="javascript:void(0);" class="reporting" data-formid="${$output['id']}">
				// 													<span class="badge badge-danger badge-query-pen mt-1">
				// 														Query Pending <span class="bg-white code-style">${pending}</span>
				// 													</span>
				// 												</a> |
				// 												<a href="javascript:void(0);" class="reporting" data-formid="${$output['id']}">
				// 													<span class="badge badge-danger badge-query-res mt-1">
				// 														Query Responded <span class="bg-white code-style">${responded}</span>
				// 													</span>
				// 												</a><br/><br/>`;
				// 											}
				// 										} else HTML += `<span class="text-danger">Report not submitted.</span>`;
				// 									HTML += `</td>
				// 								</tr>
				// 							</tbody>`;
				// 							if($output['indicator_list'].length > 0) {
				// 								HTML += `<tbody>
				// 									<tr>
				// 										<td colspan="6">
				// 											<div class="table-responsive">
				// 												<table class="table table-bordered table_th m-0">
				// 													<thead>
				// 														<tr>
				// 															<th></th>
				// 															<th>Indicator Name</th>
				// 															<th></th>
				// 														</tr>
				// 													</thead>`;
				// 													$output['indicator_list'].forEach(function($indicator, $key) {
				// 													HTML += `<tbody>
				// 														<tr class="bg-white" style="cursor: auto">`;
				// 															if($indicator['subindicator_list'].length > 0) {
				// 															HTML += `<td>
				// 																<i class="fa fa-chevron-right collapsed" data-toggle="collapse" data-target="#indicator${$indicator['id']}"></i>
				// 															</td>`;
				// 															} else {
				// 															HTML += `<td></td>`;
				// 															}
				// 															HTML += `<td class="output_text">${$indicator['title']}</td>
				// 															<td data-type="indicator" data-id="${$indicator['id']}"></td>
				// 														</tr>
				// 													</tbody>`;
																	
				// 													if($indicator['subindicator_list'].length > 0) {
				// 													HTML += `<tbody class="collapse" id="indicator${$indicator['id']}">`;
				// 														$indicator['subindicator_list'].forEach(function($subindicator, $key) {
				// 														HTML += `<tr style="cursor: auto">
				// 															<td></td>
				// 															<td>${$subindicator['title']}</td>
				// 															<td data-type="subindicator" data-id="${$subindicator['id']}"></td>
				// 														</tr>`;
				// 														});
				// 													HTML += `</tbody>`;
				// 													}
				// 													});
				// 												HTML += `</table>
				// 											</div>
				// 										</td>
				// 									</tr>
				// 								</tbody>`;
				// 							}
				// 						});
				// 					HTML += `</table>
				// 				</div>
				// 			</div>
				// 		</div>
				// 	</div>
				// </div>`;
				// $('#po'+id).html(HTML);


				var HTML = `<div class="row">
					<div class="col-md-12 col-lg-12">
						<div class="card card-block">
							
							<div class="card-body mb-5">
								<div class="table-responsive">
									<table class="table table-bordered table_th">`;
									$po['output_list'].forEach(function($output, $key) {
										HTML += `<thead>
											<tr>
												<th style="text-align:justify;padding:15px;line-height:2;background:#EAF3EF;">`+$output.title
													if($output['report'].length == 0) {
														HTML += `<strong class="text-danger pull-right">Report not submitted</strong>`;
													} else {
														HTML += `<strong class="text-success pull-right collapsed" data-toggle="collapse" data-target="#output${$output['id']}">
															Report submitted <i class="fa fa-plus"></i>
														</strong>`;
													}
												HTML += `</th>
											</tr>
										</thead>`;
										if($output['indicator_list'].length > 0) {
										HTML += `<tbody>`;
											if($output['report'].length > 0) {
											HTML += `<tr class="collapse" id="output${$output['id']}">
												<td>`;
												for(var report of $output['report']){
													HTML += `<div style="margin-top:20px;">`;
													HTML += report['report']+" by "+report['username'];
													var pending = 0, responded = 0;
													if(report['query_status']) {
														if(report['query_status'] == 1) { pending = 1;responded = 0; }
														if(report['query_status'] == 2) { pending = 0;responded = 1; }
													}
													HTML += `<a class="text-primary reporting ml-5" data-formid="${$output['id']}" href="javascript:void(0);">
														<span class="text-success">
															View Details 
														</span>
													</a> |`;
													if(user_role == 6) {
													HTML += `<a class="text-primary delete" data-id="${report['data_id']}" href="javascript:void(0);">
														<span class="text-danger">
															Delete
														</span>
													</a> |`;
													}
													HTML += `<a href="javascript:void(0);" class="reporting" data-formid="${$output['id']}">
														<span class="badge badge-danger badge-query-pen mt-1">
															Query Pending <span class="bg-white code-style">${pending}</span>
														</span>
													</a> |
													<a href="javascript:void(0);" class="reporting" data-formid="${$output['id']}">
														<span class="badge badge-danger badge-query-res mt-1">
															Query Responded <span class="bg-white code-style">${responded}</span>
														</span>
													</a></div>`;
												}
												HTML += `</td>
											</tr>`;
											}
											HTML += `<tr>
												<td colspan="6">
													<div class="table-responsive">
														<table class="table table-bordered table_th m-0">
															<thead>
																<tr>
																	<td></td>
																	<td style="width:45%;"></td>
																	<td></td>
																</tr>
															</thead>`;
															$output['indicator_list'].forEach(function($indicator, $key) {
															HTML += `<tbody>
																<tr class="bg-white" style="cursor: auto">`;
																	if($indicator['subindicator_list'].length > 0) {
																		HTML += `<td>
																			<i class="fa fa-chevron-right collapsed" data-toggle="collapse" data-target="#indicator${$indicator['id']}"></i>
																		</td>`;
																	} else {
																		HTML += `<td style="width: 3.1%;"></td>`;
																	}
																	HTML += `<td class="output_text">${$indicator['title']}</td>
																	<td data-type="indicator" data-id="${$indicator['id']}"></td>
																</tr>
															</tbody>`;
															
															if($indicator['subindicator_list'].length > 0) {
															HTML += `<tbody class="collapse" id="indicator${$indicator['id']}">`;
																$indicator['subindicator_list'].forEach(function($subindicator, $key) {
																HTML += `<tr style="cursor: auto">
																	<td></td>
																	<td>${$subindicator['title']}</td>
																	<td data-type="subindicator" data-id="${$subindicator['id']}"></td>
																</tr>`;
																});
															HTML += `</tbody>`;
															}
															});
														HTML += `</table>
													</div>
												</td>
											</tr>
										</tbody>`
										};
									});
									HTML += `</table>
								</div>
							</div>
						</div>
					</div>
				</div>`;
				$('#po'+id).html(HTML);
				getPoDetailsCounters();
			}
		});
	}

	function getPoDetailsCounters() {
		// <a href="saved_reports.html">
		// 	<span class="badge badge-saved">
		// 		Saved <span class="bg-white code-style">
		// 			<span class="loading"></span>
		// 		</span>
		// 	</span>
		// </a> |
		// <a href="reports.html">
		// 	<span class="badge badge-submit">
		// 		Submitted <span class="bg-white code-style">
		// 			<span class="loading"></span>
		// 		</span>
		// 	</span>
		// </a> |
		// <a href="pending_approval_reports.html">
		// 	<span class="badge badge-approve-pen">
		// 		Pending Approval <span class="bg-white code-style">
		// 			<span class="loading"></span>
		// 		</span>
		// 	</span>
		// </a> |

		var poid = $('a[data-toggle="tab"].active').data('id');
		var HTML = `<a href="javascript:void(0);">
			<span class="badge badge-approve">
				Approved <span class="bg-white code-style">
					<span class="loading"></span>
				</span>
			</span>
		</a> |

		<a href="javascript:void(0);">
			<span class="badge badge-danger badge-query-pen">
				Query Pending <span class="bg-white code-style">
					<span class="loading"></span>
				</span>
			</span>
		</a> |
		<a href="javascript:void(0);">
			<span class="badge badge-danger badge-query-res">
				Query Responded <span class="bg-white code-style">
					<span class="loading"></span>
				</span>
			</span>
		</a>`;
		$('body').find('[data-type="indicator"], [data-type="subindicator"]').html(HTML);

		// AJAX to get po details counter values
		$.ajax({
			url: '<?php echo base_url(); ?>helper/get_po_details_counter',
			type: 'POST',
			dataType:'JSON',
			data: {
				po: poid,
				purpose: 'review',
				year: $('[name="year"]').val(),
				crop: $('[name="crop"]').val(),
				country: $('[name="country"]').val()
			},
			error: function() {
				// $.toast({
				// 	stack: false,
				// 	icon: 'error',
				// 	position: 'bottom-right',
				// 	showHideTransition: 'slide',
				// 	heading: 'Network Error!',
				// 	text: 'Please check your internet connection.'
				// });
				var HTML = `<a href="approve.html">
					<span class="badge badge-approve">
						Approved <span class="bg-white code-style">N/A</span>
					</span>
				</a> |

				<a href="javascript:void(0);">
					<span class="badge badge-danger badge-query-pen">
						Query Pending <span class="bg-white code-style">N/A</span>
					</span>
				</a> |
				<a href="javascript:void(0);">
					<span class="badge badge-danger badge-query-res">
						Query Responded <span class="bg-white code-style">N/A</span>
					</span>
				</a>`;
				$('body').find('[data-type="indicator"], [data-type="subindicator"]').html(HTML);
			},
			success: function(response) {
				if(response.status == 0) {
					$.toast({
						stack: false,
						icon: 'error',
						position: 'bottom-right',
						showHideTransition: 'slide',
						heading: 'Error!',
						text: response.msg
					});
					return false;
				}

				var year_id = $('[name="year"]').val(),
				crop_id = $('[name="crop"]').val(),
				country_id = $('[name="country"]').val();
				
				var HTML = `<a href="javascript:void(0);">
					<span class="badge badge-approve">
						Approved <span class="bg-white code-style">N/A</span>
					</span>
				</a> |

				<a href="javascript:void(0);">
					<span class="badge badge-danger badge-query-pen">
						Query Pending <span class="bg-white code-style">N/A</span>
					</span>
				</a> |
				<a href="javascript:void(0);">
					<span class="badge badge-danger badge-query-res">
						Query Responded <span class="bg-white code-style">N/A</span>
					</span>
				</a>`;
				$('body').find('[data-type="indicator"], [data-type="subindicator"]').html(HTML);

				var user_id = <?php echo $this->session->userdata('login_id'); ?>;

				console.log(response);

				response.subindicators.forEach(function(indi, index) {
					var links = {
						approved: `<?php echo base_url(); ?>dashboard/form_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/3/` + user_id,
						query_pen: `<?php echo base_url(); ?>dashboard/query_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/1/` + user_id,
						query_res: `<?php echo base_url(); ?>dashboard/query_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/2/` + user_id
					};

					var HTML = `<a href="${((indi.approved == 0) ? 'javascript:void(0);' : links.approved)}" target="${((indi.approved == 0) ? '' : '_blank')}">
						<span class="badge badge-approve">
							Approved <span class="bg-white code-style">${indi.approved}</span>
						</span>
					</a> |

					<a href="${((indi.query_pen == 0) ? 'javascript:void(0);' : links.query_pen)}" target="${((indi.query_pen == 0) ? '' : '_blank')}">
						<span class="badge badge-danger badge-query-pen">
							Query Pending <span class="bg-white code-style">${indi.query_pen}</span>
						</span>
					</a> |
					<a href="${((indi.query_res == 0) ? 'javascript:void(0);' : links.query_res)}" target="${((indi.query_res == 0) ? '' : '_blank')}">
						<span class="badge badge-danger badge-query-res">
							Query Responded <span class="bg-white code-style">${indi.query_res}</span>
						</span>
					</a>`;
					$('body').find('[data-type="subindicator"][data-id="' + indi.id + '"]').html(HTML);
				});

				response.indicators.forEach(function(indi, index) {
					// Get sub indicator container
					var subContainer = $('body').find('[data-type="indicator"][data-id="' + indi.id + '"]');
					subContainer = subContainer.closest('tbody').next('tbody#indicator' + indi.id);
					var target = '_blank',
						links = {
							approved: `<?php echo base_url(); ?>dashboard/form_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/3`,
							query_pen: `<?php echo base_url(); ?>dashboard/query_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/1`,
							query_res: `<?php echo base_url(); ?>dashboard/query_data/` + year_id + `/`+country_id+`/`+crop_id+`/` + indi.id + `/2`
						};

					target = '_blank';
					if (indi.approved == 0) {
						target = '';
						links.approved = 'javascript:void(0);';
						subContainer.find('.badge-approve').each(function(index) {
							indi.approved = parseInt(indi.approved) + parseInt($(this).find('span').html());
						});
					}
					var HTML = `<a href="${links.approved}" target="${target}">
						<span class="badge badge-approve">
							Approved <span class="bg-white code-style">${indi.approved}</span>
						</span>
					</a> |`;

					target = '_blank';
					if (indi.query_pen == 0) {
						target = '';
						links.query_pen = 'javascript:void(0);';
						subContainer.find('.badge-query-pen').each(function(index) {
							indi.query_pen = parseInt(indi.query_pen) + parseInt($(this).find('span').html());
						});
					}
					HTML += `<a href="${links.query_pen}" target="${target}">
						<span class="badge badge-danger badge-query-pen">
							Query Pending <span class="bg-white code-style">${indi.query_pen}</span>
						</span>
					</a> |`;

					target = '_blank';
					if (indi.query_res == 0) {
						target = '';
						links.query_res = 'javascript:void(0);';
						subContainer.find('.badge-query-res').each(function(index) {
							indi.query_res = parseInt(indi.query_res) + parseInt($(this).find('span').html());
						});
					}
					HTML += `<a href="${links.query_res}" target="${target}">
						<span class="badge badge-danger badge-query-res">
							Query Responded <span class="bg-white code-style">${indi.query_res}</span>
						</span>
					</a>`;
					$('body').find('[data-type="indicator"][data-id="' + indi.id + '"]').html(HTML);
				});
			}
		});
	}

	$('body').on('click', '.reporting', function(){
		var year_id = $('[name="year"]').val();
		var country_id = $('[name="country"]').val();
		var crop_id = $('[name="crop"]').val();

		var form_id = $(this).data('formid');

		if(country_id != '' && crop_id != ''){
			var url = "<?php echo base_url(); ?>reporting/review_data/"+form_id+"/"+year_id+"/"+country_id+"/"+crop_id+"";
			window.open(url, '_blank');
		}else{
			/*if(country_id > 1 && crop_id.length > 1){
				var msg = 'Please select only one country and crop for reporting data.';
			}else if(country_id.length > 1){
				var msg = 'Please select only one country for reporting data.';
			}else if(crop_id.length > 1){
				var msg = 'Please select only one crop for reporting data.';
			}else if(country_id.length == 0 && crop_id.length == 0){
				var msg = 'Please select one country and crop for reporting data.';
			}*/
			var msg = 'Please select one country and crop for reviewing data.';

			$.toast({
				stack: false,
				icon: 'warning',
				position: 'bottom-right',
				showHideTransition: 'slide',
				heading: 'Warning!',
				text: msg
			});
		}
	});

	// Handle delete on click
	$('body').on('click', '.delete', function(){
		var recordid = $(this).data('id');

		swal({
			title: "Are you sure?",
			text: "Please enter the reason for deleting of this record!",
			type: "input",
			showCancelButton: true,
			closeOnConfirm: false,
			inputPlaceholder: "Reason"
		}, function (inputValue) {
			if (inputValue === false) return false;
			if (inputValue === "") {
				swal.showInputError("You need to provide a reason!");
				return false
			}

			$.ajax({
				url: "<?php echo base_url(); ?>dashboard/delete_data",
				type: "POST",
				dataType: "json",
				data : {
					recordid : recordid,
					reason : inputValue
				},
				error : function(){
					swal({
						title: 'Something went wrong please try after sometime.',
						icon: "warning",
						dangerMode : true,
						closeOnConfirm: true
					});
				},
				success : function(response) {
					if(response.status == 0) {
						swal({
							title: response.msg,
							icon: "warning",
							dangerMode : true,
							closeOnConfirm: true
						});
					} else {
						swal({
							title: response.msg,
							icon: "success",
							closeOnConfirm: true
						});
						location.reload(true);
					}
				}
			});
		});
	});
</script>